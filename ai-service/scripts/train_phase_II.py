...
def main():
    ...
    run_name = (
        "efficientnet_b0_phase2_dry_run"
        if DRY_RUN
        else "efficientnet_b0_phase2"
    )
    ...
    print("AutoSpot phase 2 fine-tuning")
    print(
    "Loaded phase 1 checkpoint "
    f"from epoch "
    f"{phase_1_checkpoint['epoch']}"
    )

    print(
        "Phase 1 validation top-1: "
        f"{phase_1_checkpoint['validation_metrics']['top_1_accuracy']:.2%}"
    )

    print(
        "Phase 1 validation top-3: "
        f"{phase_1_checkpoint['validation_metrics']['top_3_accuracy']:.2%}"
    )
